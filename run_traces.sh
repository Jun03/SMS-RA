#!/bin/sh
#./build_champsim.sh bimodal no no no lru 2
#./build_champsim.sh bimodal no no no lru-no-cce 2
...
do 
	...
	if [ $i -eq 5 ]
   	then
		#./run_2core.sh bimodal-no-no-no-lru-2core 1 10 $i $f
		#./run_2core.sh bimodal-no-no-no-lru-no-cce-2core 1 10 $i $f
