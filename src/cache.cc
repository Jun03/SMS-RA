...
void CACHE::handle_fill()
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
		...
		cpu = 0;
            ...
            if (cache_type == IS_LLC)
            {
                if(block[set][way].cpu == MSHR.entry[mshr_index].cpu && block[set][way].valid)
                {
                    ice_count[MSHR.entry[mshr_index].cpu]++;
                }
                else if(block[set][way].cpu != MSHR.entry[mshr_index].cpu && block[set][way].valid)
                {
                    cce_count[MSHR.entry[mshr_index].cpu]++;
                }
            }
	  ...
	  /*  if (cache_type == IS_L2C || cache_type == IS_L1D || cache_type == IS_LLC)//Write no-allocate for write-back packets  
        ...
        if(cache_type == IS_L2C && block[set][way].valid)
        {
			...
			cpu = 0;
					...
					if (cache_type == IS_LLC)
                    {
                        if(block[set][way].cpu == WQ.entry[index].cpu && block[set][way].valid)
                        {
                            ice_count[WQ.entry[index].cpu]++;
                        }
                        else if(block[set][way].cpu != WQ.entry[index].cpu && block[set][way].valid)
                        {
                            cce_count[WQ.entry[index].cpu]++;
                        }
                    }
